...
First set up an
edge condition: the maximum of a singleton list is just its only element. The edge condition is where the recursive loop will terminate.
The maximum of a longer list is either its single head element, or the maximum of the rest of the list (the tail).
...
As you can see, pattern matching goes great with recursion! Haskell fares better
than imperative languages here because we didn't need to write (or think through) a long chain of if-else's.

We used pattern
matching to split the list into its head and tail. This is a very common
...
Let's check out how this would work
on an example list: [2,5,1]. If we call maximum' on that, the first two patterns
won't match. The third one will so the list splits into 2 and [5,1].
The routine gets to 2 > maxTail then needs to find out what maxTail is.
So it goes to the *where* clause.
The *where* clause wants to know the maximum' of [5,1], so we run maximum'
on the new shorter input [5,1]. Again the third pattern matchesso [5,1] is split into 5 and
...
is 5, we choose 5. That's the answer.
...
How's that for elegant! In essence, the maximum of a list is the larger of
